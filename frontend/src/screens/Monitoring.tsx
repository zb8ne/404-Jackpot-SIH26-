...
export function ValidationHeader({ title, busy, onRefresh }: { title: string; busy: boolean; onRefresh: () => void }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-4">
      ...
      <button type="button" onClick={onRefresh} disabled={busy} className="rounded-md border-2 border-slate-700 px-4 py-2 font-bold uppercase tracking-wide text-slate-300 shadow-[3px_3px_0_0_rgba(100,116,139,0.3)] transition-all duration-100 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:border-slate-500 active:translate-x-0 active:translate-y-0 active:shadow-none disabled:opacity-50 disabled:shadow-none">{busy ? 'Loading…' : 'Refresh'}</button>
    </div>
  )
}
