  ...
  return <LifecycleWorkspace eyebrow="Lifecycle control" title="Revoke a credential" description="End the validity of an existing department credential. Its issuance history remains permanently verifiable." tone="rose">
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(300px,.75fr)]">
      <form onSubmit={submit} className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/50">
        <div className="border-b border-slate-800 bg-slate-900/40 p-6"><StepTitle number="01" title="Find an existing credential" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by document ID or citizen name…" className="mt-4 w-full rounded-md border-2 border-slate-700 bg-slate-950 px-4 py-3 text-slate-100 outline-none placeholder:text-slate-600 focus:border-rose-500" /></div>
        <div className="max-h-80 space-y-2 overflow-y-auto p-4">
          {filtered.map((credential) => <button key={credential.docHash} type="button" onClick={() => setDocHash(credential.docHash)} className={`w-full rounded-md border-2 p-4 text-left transition-all duration-100 hover:-translate-x-0.5 hover:-translate-y-0.5 active:translate-x-0 active:translate-y-0 active:shadow-none ${docHash === credential.docHash ? 'border-rose-500/60 bg-rose-500/10 shadow-[3px_3px_0_0_rgba(244,63,94,0.4)]' : 'border-slate-700 bg-slate-900/40 shadow-[3px_3px_0_0_rgba(100,116,139,0.25)] hover:border-slate-500'}`}><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-sm font-bold text-slate-100">{credential.docId}</p><p className="mt-1 text-sm text-slate-400">{credential.citizen}</p></div><span className="rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-bold text-emerald-300">VALID</span></div></button>)}
          {filtered.length === 0 && <div className="py-8 text-center text-sm text-slate-500">No matching valid credentials.</div>}
        </div>
        <div className="border-t border-slate-800 p-6"><StepTitle number="02" title="Confirm revocation" />{selected ? <div className="mt-4 rounded-xl border border-rose-500/20 bg-rose-500/5 p-4"><p className="text-sm text-slate-300">You are revoking <span className="font-mono font-bold text-rose-300">{selected.docId}</span>.</p><p className="mt-2 text-xs leading-5 text-slate-500">This changes its on-chain status to REVOKED. The credential will remain discoverable and its history will not be deleted.</p></div> : <p className="mt-4 text-sm text-slate-500">Select a credential above to continue.</p>}<button type="submit" disabled={busy || !department || !docHash} className="mt-5 w-full rounded-md border-2 border-rose-900 bg-rose-500 px-6 py-4 font-bold uppercase tracking-wide text-white shadow-[4px_4px_0_0_rgba(244,63,94,0.5)] transition-all duration-100 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-rose-400 active:translate-x-0 active:translate-y-0 active:shadow-none disabled:cursor-not-allowed disabled:border-slate-700 disabled:bg-slate-700 disabled:text-slate-400 disabled:shadow-none">{busy ? 'Submitting revocation…' : 'Revoke selected credential'}</button></div>
      </form>
    ...
    <LifecycleWorkspace eyebrow="Credential correction" title="Supersede a credential" description="Anchor a corrected PDF while preserving the original credential as part of its permanent history." tone="amber">
      ...
      <form onSubmit={submit} className="space-y-6 rounded-2xl border border-slate-800 bg-slate-950/50 p-6 sm:p-8">
        ...
        <label className="block">
          ...
          <select value={citizenAccountId} onChange={(e) => { const id = e.target.value; setCitizenAccountId(id); setCitizen(citizenOptions.find((item) => item.id === id)?.displayName ?? '') }} className="w-full rounded-md border-2 border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
        ...
        <label className="block">
          ...
          <input type="file" accept="application/pdf" required onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="w-full rounded-md border-2 border-slate-700 bg-slate-950 p-3 text-slate-300 file:mr-4 file:rounded file:border-0 file:bg-slate-800 file:px-4 file:py-2 file:text-slate-200" />
        </label>
        <button type="submit" disabled={busy || !department || !file || !oldHash || !docId.trim()} className="w-full rounded-md border-2 border-amber-900 bg-amber-400 px-6 py-4 font-bold uppercase tracking-wide text-amber-950 shadow-[4px_4px_0_0_rgba(245,158,11,0.5)] transition-all duration-100 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-amber-300 active:translate-x-0 active:translate-y-0 active:shadow-none disabled:border-slate-700 disabled:bg-slate-700 disabled:text-slate-400 disabled:shadow-none">{busy ? 'Anchoring replacement…' : 'Supersede & anchor replacement'}</button>
      </form>
...
function TextField({ label, value, onChange, placeholder, mono }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; mono?: boolean }) {
  return <label className="block"><span className="mb-2 block text-xs font-semibold uppercase tracking-widest text-slate-500">{label}</span><input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} required className={`w-full rounded-md border-2 border-slate-700 bg-slate-950 px-4 py-3 text-slate-100 outline-none focus:border-sky-500 ${mono ? 'font-mono' : ''}`} /></label>
}
