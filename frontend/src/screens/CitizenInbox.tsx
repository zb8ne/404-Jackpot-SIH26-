...
export function CitizenInbox() {
  ...
  return <section className="space-y-6">
    ...
    <div className="space-y-4">{requests.map((request) => <article key={request.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
      ...
      {request.state === 'PENDING' && <div className="mt-6 flex flex-wrap gap-3">
        <button type="button" onClick={() => void decide(request.id, 'APPROVED')} className="rounded-md border-2 border-emerald-900 bg-emerald-600 px-5 py-3 font-bold uppercase tracking-wide text-white shadow-[4px_4px_0_0_rgba(16,185,129,0.5)] transition-all duration-100 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-emerald-500 active:translate-x-0 active:translate-y-0 active:shadow-none">Approve request</button>
        <button type="button" onClick={() => void decide(request.id, 'DENIED')} className="rounded-md border-2 border-red-500/50 px-5 py-3 font-bold uppercase tracking-wide text-red-300 shadow-[4px_4px_0_0_rgba(239,68,68,0.35)] transition-all duration-100 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-red-500/10 active:translate-x-0 active:translate-y-0 active:shadow-none">Deny request</button>
      </div>}
